...
const fastcsv = require("fast-csv"); // Corrected to import fast-csv correctly
...
// Generate PDF Report
router.get("/pdf/:horseId", async (req, res) => {
  try {
...
// Generate CSV Report
router.get("/csv/:horseId", async (req, res) => {
  try {
    const { horseId } = req.params;
    const horse = await getHorseData(horseId);
...
router.get("/excel/:horseId", async (req, res) => {
  try {
    const { horseId } = req.params;
    const horse = await getHorseData(horseId);
